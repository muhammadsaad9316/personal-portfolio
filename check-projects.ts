...
async function checkProjects() {
    ...
    try {
        ...
        if (allProjects.length === 0) {
            ...
            console.log('👉 You need to create projects via http://localhost:3000/admin/projects\n');
            return;
        }
        ...
        if (featuredProjects.length === 0) {
            ...
            console.log('👉 You need to check the "Feature this project on homepage" checkbox when creating/editing projects\n');
        ...
        categories.forEach(cat => {
            ...
            console.log(`  - "${cat}" (${count} project${count === 1 ? '' : 's'})`);
        });
        ...
        if (error.message?.includes('category')) {
            console.log('\n👉 The "category" field is not in your Prisma Client yet!');
